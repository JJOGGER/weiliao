...
public class MessageListPanelEx {
    ...
    public void onRedPacketEvent(RedPacketEvent event) {
        if (event.getAction() == RedPacketEvent.ACTION_SEND_RED_PACKET) {
            IMMessage message = (IMMessage) event.getSerializableExtra(Extras.MESSAGE);
            container.proxy.sendMessage(message);
        } else if (event.getAction() == RedPacketEvent.ACTION_READ_RED_PACKET) {
    ...
    private class MsgItemEventListener implements MsgAdapter.ViewHolderEventListener {
        ...
        private void longClickItemForwardToPerson(final IMMessage item, CustomAlertDialog alertDialog) {
            alertDialog.addItem(container.activity.getString(R.string.forward_to_person), new CustomAlertDialog.onSeparateItemClickListener() {
                ...
                public void onClick() {
                    if (isForbidden) {
                        ToastHelper.showToast(container.activity, "群主已开启了群禁言");
                        return;
                    }
                    forwardMessage = item;
