...
public class AdvancedTeamInfoActivity extends UI implements
    ...
    private SwitchButton mForbiddenSwitchButton;
    private SwitchButton mSingleChatSwitchButton;
    private View layoutSingleChat;
    private TeamActionInfo mTeamActionInfo;
    ...
    private void findViews() {
        ...
        layoutSingleChat = findViewById(R.id.single_chat_layout);
        mSingleChatSwitchButton = layoutSingleChat.findViewById(R.id.single_chat);
        mSingleChatSwitchButton.setTag("singleChat");
        ((TextView) layoutTeamExtension.findViewById(R.id.item_title)).setText("群禁言");
        ((TextView) layoutSingleChat.findViewById(R.id.single_chat_title)).setText("允许普通成员私聊");
        mForbiddenSwitchButton = layoutTeamExtension.findViewById(R.id.group_forbidden);
        mForbiddenSwitchButton.setTag("forbidden");
...
//                mTeamActionInfo.setForbidden(checkState ? 1 : 0);
//                String state = TeamActionInfo.toGson(mTeamActionInfo);
...
//                        saved();
                    ...
                    public void onFailed(int code) {
                        if (code == ResponseCode.RES_TEAM_ENACCESS) {
                            ToastHelper.showToast(AdvancedTeamInfoActivity.this, R.string.no_permission);
...
//                NIMClient.getService(TeamService.class).updateTeam(teamId, TeamFieldEnum.AllMute, state).setCallback(new RequestCallback<Void>() {
//                    @Override
//                    public void onSuccess(Void param) {
//                        ToastHelper.showToast(AdvancedTeamInfoActivity.this, R.string.update_success);
////                        saved();
...
//                    }
//                });
            } else if ("singleChat".equals(key)) {
                mTeamActionInfo.setAllowSingleChat(checkState ? 1 : 0);
                String state = TeamActionInfo.toGson(mTeamActionInfo);
                Log.e(TAG, "------------------JSON:" + state);
                NIMClient.getService(TeamService.class).updateTeam(teamId, TeamFieldEnum.Extension, state).setCallback(new RequestCallback<Void>() {
                    @Override
                    public void onSuccess(Void param) {
                        ToastHelper.showToast(AdvancedTeamInfoActivity.this, R.string.update_success);
//                        saved();
    ...
    private void updateTeamInfo(final Team t) {
        ...
        if (team == null) {
            ...
            if (creator.equals(NimUIKit.getAccount())) {
                isSelfAdmin = true;
                layoutTeamExtension.setVisibility(View.VISIBLE);
                layoutSingleChat.setVisibility(View.VISIBLE);
            } else {
                layoutTeamExtension.setVisibility(View.GONE);
                layoutSingleChat.setVisibility(View.GONE);
            }
...
//        teamIdText.setText(team.getId());
        ...
        mForbiddenSwitchButton.setOnChangedListener(null);
        mSingleChatSwitchButton.setOnChangedListener(null);
        mTeamActionInfo = TeamActionInfo.fromGson(team.getExtension());
        Log.e(TAG, "-------->TEAM:" + team.isAllMute());
        mForbiddenSwitchButton.setCheck(team.isAllMute());
        mSingleChatSwitchButton.setCheck(mTeamActionInfo.isAllowSingleChat() == 1);
        mForbiddenSwitchButton.setOnChangedListener(onChangedListener);
        mSingleChatSwitchButton.setOnChangedListener(onChangedListener);
