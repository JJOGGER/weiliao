...
public abstract class AbsContactDataList {
    ...
    protected final Map<String, Group> groupMap = new HashMap<>();//hashmap会按字母自动排序
    protected final Map<String, Group> groupMap2 = new LinkedHashMap<>();
    ...
    public AbsContactDataList(ContactGroupStrategy groupStrategy, boolean isSort) {
    ...
    public final void add(AbsContactItem item, boolean isSort) {
        if (item == null) {
            return;
        }
        ...
        if (id == null) {
            ...
            if (!isSort) {
                group = groupMap2.get(id);
                if (group == null) {
                    group = new Group(id, groupStrategy.getName(id));
                    groupMap2.put(id, group);
                }
            } else {
