...
//                    RecentContact recent = adapter.getData().get(j);
//                    if (adapter.getData().get(j).getRecentMessageId().equals(accounts.get(i))){
//                        NIMClient.getService(MsgService.class).deleteRecentContact(recent);
//                        NIMClient.getService(MsgService.class).clearChattingHistory(recent.getContactId(), recent.getSessionType());
//                        adapter.remove(j);
//                        postRunnable(new Runnable() {
    ...
    ContactChangedObserver friendDataChangedObserver = new ContactChangedObserver() {
        ...
        public void onDeletedFriends(List<String> accounts) {
            ...
            for (int i = 0; i < accounts.size(); i++) {
                for (int j = 0; j < adapter.getData().size(); j++) {
                    RecentContact recent = adapter.getData().get(j);
                    if (adapter.getData().get(j).getContactId().equals(accounts.get(i))) {
                        NIMClient.getService(MsgService.class).deleteRecentContact(recent);
                        NIMClient.getService(MsgService.class).clearChattingHistory(recent.getContactId(), recent.getSessionType());
                        adapter.remove(j);
                        postRunnable(new Runnable() {
