...
public class PagerSlidingTabStrip extends HorizontalScrollView implements OnPageChangeListener {
    ...
    public void onPageSelected(int position) {
        if (position == 3 && onTabClickListener != null) {
    ...
    private void setChooseTabViewTextColor(int position) {
        ...
        for (int i = 0; i < childCount; ++i) {
            tabView = (FrameLayout) tabsContainer.getChildAt(i);
            textView = (TextView) tabView.findViewById(R.id.tab_title_label);
            ivIcon = tabView.findViewById(R.id.iv_icon);
            if (i == position) {
    ...
    private void addTabView(final int position, String title) {
        ...
        switch (title) {
            case "会话":
                icon.setImageResource(R.drawable.selector_actionbar_msg);
                break;
            case "通讯录":
                icon.setImageResource(R.drawable.selector_actionbar_contact);
                break;
            case "发现":
                icon.setImageResource(R.drawable.selector_actionbar_find);
                break;
            case "我的":
                icon.setImageResource(R.drawable.selector_actionbar_me);
                break;
        }
    ...
    public void updateTab(int index, ReminderItem item) {
        FrameLayout tabView = (FrameLayout) tabsContainer.getChildAt(index);
