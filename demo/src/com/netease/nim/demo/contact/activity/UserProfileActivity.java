...
public class UserProfileActivity extends UI {
    ...
    private RelativeLayout setGroupLayout;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        accid = getIntent().getStringExtra(Extras.EXTRA_ACCOUNT);
    ...
    private void findViews() {
        ...
        setGroupLayout = findView(R.id.set_group);
        ...
        aliasLayout.setOnClickListener(v -> UserProfileEditItemActivity.startActivity(UserProfileActivity.this, UserConstant.KEY_ALIAS, accid));
        setGroupLayout.setOnClickListener(v -> SetGroupActivity.startActivity(UserProfileActivity.this, UserConstant.KEY_ALIAS, accid));
    }

    private void initActionbar() {
        TextView toolbarView = findView(R.id.action_bar_right_clickable_textview);
        if (!TextUtils.equals(accid, DemoCache.getAccount())) {
        ...
        toolbarView.setOnClickListener(v -> UserProfileSettingActivity.start(UserProfileActivity.this, accid));
    }
    ...
    private void updateUserInfo() {
        if (NimUIKit.getUserInfoProvider().getUserInfo(accid) != null) {
            updateUserInfoView();
            return;
        }

        NimUIKit.getUserInfoProvider().getUserInfoAsync(accid, new SimpleCallback<NimUserInfo>() {
    ...
    private void updateUserInfoView() {
        mUserDataSource.getAccount(accid, new RequestMultiplyCallback<AccountResult>() {
            @Override
            ...
            public void onSuccess(AccountResult accountResult) {
                accountText.setText("帐号：" + accountResult.getAccount());
                headImageView.loadBuddyAvatar(accid);
                if (TextUtils.equals(accid, DemoCache.getAccount())) {
                    nameText.setText(UserInfoHelper.getUserName(accid));
                }

                final NimUserInfo userInfo = (NimUserInfo) NimUIKit.getUserInfoProvider().getUserInfo(accid);
                if (userInfo == null) {
...
//            removeFriendBtn.setVisibility(View.VISIBLE);
    ...
    private void updateToggleView() {
        if (DemoCache.getAccount() != null && !DemoCache.getAccount().equals(accid)) {
            ...
            if (NIMClient.getService(FriendService.class).isMyFriend(accid)) {
                RecentContact recentContact = NIMClient.getService(MsgService.class).queryRecentContact(accid, SessionTypeEnum.P2P);
    ...
    private void updateAlias(boolean isFriend) {
        if (isFriend) {
            ...
            setGroupLayout.setVisibility(View.VISIBLE);
            String alias = NimUIKit.getContactProvider().getAlias(accid);
            String name = UserInfoHelper.getUserName(accid);
    ...
    private SwitchButton.OnChangedListener onChangedListener = new SwitchButton.OnChangedListener() {
        ...
        public void OnChanged(View v, final boolean checkState) {
            ...
            if (KEY_RECENT_STICKY.equals(key)) {
                ...
                if (checkState) {
                    ...
                    if (recentContact == null) {
                        ...
                        NIMClient.getService(MsgService.class).createEmptyRecentContact(accid,
                                SessionTypeEnum.P2P,
            ...
            if (key.equals(KEY_BLACK_LIST)) {
                if (checkState) {
                    NIMClient.getService(FriendService.class).addToBlackList(accid).setCallback(new RequestCallback<Void>() {
                        @Override
                    ...
                    NIMClient.getService(FriendService.class).removeFromBlackList(accid).setCallback(new RequestCallback<Void>() {
                        @Override
                        public void onSuccess(Void param) {
                ...
                NIMClient.getService(FriendService.class).setMessageNotify(accid, checkState).setCallback(new RequestCallback<Void>() {
                    @Override
                    public void onSuccess(Void param) {
                        if (checkState) {
    ...
    private void doAddFriend(String msg, boolean addDirectly) {
        ...
        if (!TextUtils.isEmpty(accid) && accid.equals(DemoCache.getAccount())) {
        ...
        EasyAlertDialog dialog = EasyAlertDialogHelper.createOkCancelDiolag(this, getString(R.string.remove_friend),
                ...
                new EasyAlertDialogHelper.OnDialogActionListener() {
                    ...
                    public void doOkAction() {
                        DialogMaker.showProgressDialog(UserProfileActivity.this, "", true);
                        NIMClient.getService(FriendService.class).deleteFriend(accid).setCallback(new RequestCallback<Void>() {
                            @Override
                            public void onSuccess(Void param) {
                                DialogMaker.dismissProgressDialog();
    ...
    private void onChat() {
        Log.i(TAG, "onChat");
        SessionHelper.startP2PSession(this, accid);
    }
}
