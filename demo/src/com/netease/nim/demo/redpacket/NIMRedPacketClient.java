...
//        NIMClient.getService(AuthServiceObserver.class).observeOnlineStatus(observer, true);
    ...
    private static boolean checkValid() {
        return (selfInfo = (NimUserInfo) NimUIKit.getUserInfoProvider().getUserInfo(DemoCache.getAccount())) != null;
    }
...
//            JrmfRpClient.sendSingleEnvelopeForResult(activity, targetAccount, selfInfo.getAccount(), thirdToken, selfInfo.getName(), selfInfo.getAvatar(), requestCode);
     ...
     */
    public static void startOpenRpDialog(final AppCompatActivity activity, final SessionTypeEnum sessionTypeEnum, final String briberyId, IMMessage message, final NIMOpenRpCallback cb) {
        if (!checkValid()) {
            return;
        }
        DialogMaker.showProgressDialog(activity, "");
        IRedPacketDataSource redPacketDataSource = new RedPacketDataSource();
        redPacketDataSource.getRedPacketStatus(DemoCache.getServerAccount(), briberyId, new RequestMultiplyCallback<RedPacketStatusResponse>() {
            @Override
            ...
            @Override
            public void onSuccess(RedPacketStatusResponse response) {
                DialogMaker.dismissProgressDialog();
                switch (response.getRedStatus()) {
                    case RedPacketConstant.RedStatus.STATUS_ACCEPT_NO:
                        ...
                        openRedPacketDialog.setRedPacketInfo(userInfo.getAccount(), userInfo.getName(), userInfo.getAvatar());
                        openRedPacketDialog.setOnOpenRedPacketDialogListener(new OpenRedPacketDialog.OnOpenRedPacketDialogListener() {
                            @Override
                            public void onOpen() {
                                redPacketDataSource.openPointRedPacket(DemoCache.getServerAccount(),
                                        selfInfo.getAvatar(),
                                        selfInfo.getName(),
                                        briberyId, new RequestMultiplyCallback<RedPacketStatusResponse>() {
                                            @Override
                                            public void onFail(BaseException e) {
                                                if (activity == null || activity.isFinishing())
                                                    return;
                                                ToastHelper.showToast(activity, e.getMessage());
                                                openRedPacketDialog.dismiss();
                                            }

                                            @Override
                                            public void onSuccess(RedPacketStatusResponse res) {
                                                openRedPacketDialog.dismiss();
                                                cb.sendMessage(selfInfo.getAccount(), briberyId, res.getRetainCount() == 0);
                                                message.setStatus(MsgStatusEnum.read);
                                                MsgService service = NIMClient.getService(MsgService.class);
                                                service.updateIMMessageStatus(message);
                                                EventBus.getDefault().post(new RedPacketEvent(RedPacketEvent.ACTION_READ_RED_PACKET));
                                                ChangePointRedPacketDetailsActivity.navTo(activity, res);
                                            }
                                        });
                            }
                        });
                        openRedPacketDialog.show(activity.getSupportFragmentManager(), OpenRedPacketDialog.class.getSimpleName());

                        break;
                    case RedPacketConstant.RedStatus.STATUS_ACCEPT_YES://已领取
...
//                        message.setAttachment(attachment);
                        ...
                        service.updateIMMessageStatus(message);
                        EventBus.getDefault().post(new RedPacketEvent(RedPacketEvent.ACTION_READ_RED_PACKET));
                        ChangePointRedPacketDetailsActivity.navTo(activity, response);
                        break;
                    case RedPacketConstant.RedStatus.STATUS_ACCEPT_OVER://已抢光
                        attachment = (RedPacketAttachment) message.getAttachment();
                        attachment.setRead(true);
                        message.setStatus(MsgStatusEnum.read);
                        service = NIMClient.getService(MsgService.class);
                        service.updateIMMessageStatus(message);
                        EventBus.getDefault().post(new RedPacketEvent(RedPacketEvent.ACTION_READ_RED_PACKET));
                        ChangePointRedPacketDetailsActivity.navTo(activity, response);
                        break;
                    case RedPacketConstant.RedStatus.STATUS_ACCEPT_BACK:
                        message.setStatus(MsgStatusEnum.read);
                        service = NIMClient.getService(MsgService.class);
                        service.updateIMMessageStatus(message);
                        EventBus.getDefault().post(new RedPacketEvent(RedPacketEvent.ACTION_READ_RED_PACKET));
                        ToastHelper.showToast(activity, "红包已过期");
                        break;
                }
